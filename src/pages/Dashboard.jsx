...
import { STAGES } from '../data/supabase.js';
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E4E8F0', borderRadius: 12, padding: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: rep.color_bg, color: rep.color_text, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
          {rep.initials}
        </div>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 14 }}>
        {[
          ['Assigned',   stats.assigned   || 0, '#1A56DB'],
          ['Activities', stats.activities || 0, '#8B5CF6'],
          ['Won',        stats.won        || 0, '#10B981'],
          ['Pipeline',   stats.pipeline   || 0, '#F59E0B'],
        ].map(([l, v, c]) => (
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #F1F5F9' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {Object.entries(stats.actByType).map(([type, count]) => (
              <span key={type} style={{ padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 600, background: '#EFF6FF', color: '#1A56DB' }}>
...
export default function DashboardPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const { data: acts } = await supabase.from('activities')
          .select('rep_id, type, logged_at')
          .gte('logged_at', weekStr);
  ...
  if (!reps.length || loading) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 400, color: '#94A3B8', fontSize: 13 }}>Loading dashboard...</div>;
  }
  ...
  return (
    <div style={{ padding: '16px 20px' }}>
      ...
      {/* Rep cards */}
      <div style={{ fontSize: 12, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>Rep performance — week of {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
        {reps.map(rep => (
          <RepCard key={rep.id} rep={rep} stats={repStats[rep.id] || {}} />
        ))}
      </div>
